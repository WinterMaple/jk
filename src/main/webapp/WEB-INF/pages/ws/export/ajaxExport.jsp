...
*/
	...
	var xmlHttpRequest = new ActiveXObject("Microsoft.XMLHTTP"); //创建XMLHTTP对象IE
	...
	function _back() {
		if (xmlHttpRequest.readystate == 4) { //提交完成后
			if (xmlHttpRequest.status == 200) { //调用成功
				...
				if (ret != null) {
					var customerContract = ret.getElementsByTagName("customerContract")[0].text; //获取到报运号
					...
					var destinationPort = ret.getElementsByTagName("destinationPort")[0].text;
					document.getElementById("destinationPort").innerHTML = destinationPort;

					var transportMode = ret.getElementsByTagName("transportMode")[0].text;
					document.getElementById("transportMode").innerHTML = transportMode;

					var priceCondition = ret.getElementsByTagName("priceCondition")[0].text;
